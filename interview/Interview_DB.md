...
- **트랜잭션의 특성** => **<u><mark>ACID</mark></u>**
  ...
  - **<mark>일관성 (Consistency)</mark>**
    ...
    - 트랜잭션이 수행되기 전, 수행된 후 는 데이터베이스는 일관적 상태(도메인 유효범위, 무결성 제약조건 등) 를 가진다.
...
- 동시성 제어를 하지 않고 다수의 트랜잭션을 동시에 수행할 때 생기는 문제
  ...
  - **dirty read**
    ...
    - 커밋되지 않는 데이터를 읽었는데 (나중에 rollback되면 잘못된 값을 가지게 되는...)
...
- deadlock
  ...
  - 두 개 이상의 트랜잭션이 다른 트랜잭션이 사용하고 있는 데이터에 접근하기 위해 대기

<br>

- 2-phase locking protocol (2단계 로킹 프로토콜)
