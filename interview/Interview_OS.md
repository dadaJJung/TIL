...
운영체제는 역할은 하드웨어 자원을 효울적으로 관리해서 사용자에게는 편리성을 제공해주는 것이다. 운영체제의 역할은 크게 프로세스 관리, 메인메모리 관리, 파일시스템 관리, I/O관리... 등등이 있다. 프로세스 관리에는 크게 CPU 스케쥴링과 프로세스 동기화 작업이 있다. 
...
- 실행중인 프로그램, 디스크로터 메모리에 적재되어 CPU의 할당을 받을 수 있다. 운영체제는 프로세스 생성시에 PCB를 생성하여 관리한다. 
...
- PCB에 저장되는 정보
  ...
  - 입출력 상태 정보 (열린 파일들, 할당된 입출력 장치들)
  
  - 어카운팅 정보 (CPU 사용 시간, 계저번호..)
...
- FCFS / SJF / Priority / RR / Multi-level Q .... 
...
- **여러 프로세스가 하나의 공유 자원에 접근할 때 data consistency (일관성) 를 유지하는 것**

- 동기화 문제 예시 => 은행계좌 입금 / 출금
  
  - 입금 프로세스와 출금 프로세스가 통장 잔액이라는 하나의 공유 데이터에 접근.
  
  - 이때 생각한 값과 다른 결과가 나올 수 있다.
  
  - 왜? high level language 에서는 `balance += 입금액`   `balance -= 출금액` 과 같은 연산이 하나의 연산으로 보이지만 사실, 기계어로 변환하면 메모리에서 잔액을 가져오고 입금액, 출금액을 가져오고, 연사하고 그 결과를 메모리에 다시 쓰는 등 .. 여러 단계의 작업으로 이루어져 있다. 따라서,,, 이렇게 논리적인 하나의 작업이 여러 작업으로 나누어져 실행중인 과정에 문맥교환이 일어나게 되면 이상한 값이 나올수있게됨.

- <mark>Critical Section (임계구역)</mark> => 공유 자원에 접근하는 코드 영역

- 임계구역 문제를 해결하기 위한 3가지 조건
  
  - 상호배제 (mutual exclusion)
    
    - 특정 프로세스가 임계구역 내에서 실행중일 때, 다른 프로세스는 임계구역에 들어갈 수 없다.
  
  - 진행 (progress)
    
    - 임계구역 내에서 실행중인 프로세스가 없을 때, 임계구역 진입을 요구한 프로세스는 임계구역에 들어갈 수 있어야 한다.
  
  - 한정된 대기 (bounded waiting)
    
    - 임계구역에 대한 진입을 요청했을 경우 , 무한히 기다리지 않고 유한한 시간내에 진입해야 한다. 
...
- 프로세스 동기화 도구 
  ...
  - 세마포어 (sw)
    ...
    - 세마포를 사용할 경우 다른 프로세스가 공유 자원을 사용하고 있는 경우, 공유 자원 사용을 시도한 또 다른 프로세스를 block list에 넣어둔다. 그리고 현재 공유 자원을 사용하고 있던 프로세서가 임계구역을 빠져 나올 때 블락되어있던 프로세스를 깨워서 ready list에 넣어준다. => `busy wait` 회피 (다른 프로세스가 임계구역을 빠져나올 때까지 계속 loop를 돌면서 진입을 시도할 필요가 없음)
...
- 여러 프로세스가 자원을 점유하면서 다른 자원에 접근하려고 기다리는 상황

- 식사하는 철학자 문제 => 대표적인 교착상태 예제
  
  - 상호배제 (포크를 같이 쓸 수 없다)
  
  - 비선점 (다른 사람의 포크를 빼앗을 수 없다)
  
  - 점유와 대기 (왼쪽 포크를 점유한 채로 오른쪽 포크 사용을 기다린다)
  
  - 원형대기 (원형 식탁에 둘러 앉아 있다)

- 교착상태를 해결하는 방법
  
  - 교착상태 예방
    
    - 상호배제, 비선점, 점유와 대기, 원형대기가 일어나지 않도록 함
  
  - 교착상태 회피
    
    - 교착상태가 발생하지 않을만큼의 자원만 할당.
...
- 메모리 영역
  ...
  - code : 프로그램 코드, 컴파일된 기계어, CPU는 코드 영역에 저장된 명령어를 하나씩 가져가서 처리
  
  - data : 전역변수, 스태틱 변수, 문자열 상수
  
  - stack : 지역변수, 매개변수, 리턴 어드레스 , 스택 영역의 크기는 컴파일 시 결정됨.  LIFO 구조 => 메모리의 높은 주소에서 낮은 주소의 방향으로 할당되기 때문에
  
  - heap : 사용자에 의해 메모리 공간에 동적으로 할당, 해제되는 영역, 런타임시 크기가 결정됨. FIFO 구조 => 메모리의 낮은 주소에서 높으 주소 방향으로 할당되기 때문에
